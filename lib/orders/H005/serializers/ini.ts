...
const iniOrderData = (ebicsAccount: any, key: any, xmlOptions: any) => {
	const xmlOrderData = {
		'@': {
		...
		SignaturePubKeyInfo: {
			'ds:X509Data': {
				'ds:X509Certificate': key.certificateBase64s().map((certBase64: string) => ({ '#': certBase64 })),
			},
...
const hiaOrderData = (ebicsAccount: any, keys: any, xmlOptions: any) => {
	const xmlOrderData = {
		'@': {
			'xmlns:ds': 'http://www.w3.org/2000/09/xmldsig#',
		...
		AuthenticationPubKeyInfo: {
			'ds:X509Data': {
				'ds:X509Certificate': keys.x().certificateBase64s().map((certBase64: string) => ({ '#': certBase64 })),
			},
			AuthenticationVersion: 'X002',
		},
		EncryptionPubKeyInfo: {
			'ds:X509Data': {
				'ds:X509Certificate': keys.e().certificateBase64s().map((certBase64: string) => ({ '#': certBase64 })),
			},
...
const h3kOrderData = (ebicsAccount: any, keys: any, xmlOptions: any) => {
	const xmlOrderData = {
		'@': {
			'xmlns:ds': 'http://www.w3.org/2000/09/xmldsig#',
			xmlns: 'urn:org:ebics:H005',
		...
		SignatureCertificateInfo: {
			'ds:X509Data': keys.a().certificateBase64s().map((certBase64: string) => ({
				'ds:X509Certificate': certBase64,
			})),
			SignatureVersion: 'A006',
		},
		AuthenticationCertificateInfo: {
			'ds:X509Data': keys.x().certificateBase64s().map((certBase64: string) => ({
				'ds:X509Certificate': certBase64,
			})),
			AuthenticationVersion: 'X002',
		},
		EncryptionCertificateInfo: {
			'ds:X509Data': keys.e().certificateBase64s().map((certBase64: string) => ({
				'ds:X509Certificate': certBase64,
			})),
			EncryptionVersion: 'E002',
		},
...
const process: Record<string, any> = {
	...
	H3K: {
		...
		body: (ebicsAccount: any, keys: any, xmlOptions: any) => {
			...
			// preserve CJS-vs-ESM `this.xmlOptions` (was undefined in CJS, throws in ESM): use optional chaining
			const signatureXml = orderSignature(ebicsAccount, orderData, keys.a(), (this as { xmlOptions?: unknown } | undefined)?.xmlOptions);
